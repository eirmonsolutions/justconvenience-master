	    ...
	    <div class="row">
	        ...
	        <div class="col-9 align-self-center filter-form">
	            ...
	            <div class="customize-input float-left Ml20 contest-input">
	                <select class="custom-select custom-select-set form-control bg-white border-0 custom-shadow custom-radius" id="datatable_order_status" name="status">
	            ...
	            <div class="customize-input float-left Ml20 contest-input">
	                <select class="custom-select custom-select-set form-control bg-white border-0 custom-shadow custom-radius" id="datatable_store_id" name="store_id">
	                    @if(sizeof($stores) > 0)
							<option value="">All</option>
							@foreach($stores as $key => $store)
								<option value="{{ $store->id }}" {{ (isset($params['store_id']) && !empty($params['store_id']) && $params['store_id'] == $store->id) ? "selected" : '' }}>{{ $store->name }}</option>
							@endforeach
						@endif
	                </select>
	            </div>
		        ...
		        <div class="card">
		            <div class="card-body">
						...
						<div class="table-responsive">
							<table id="orders"  class="table no-wrap v-middle mb-0 dashboard-table responsive nowrap">
								<thead>
									<tr class="border-0">             
	                                    <th class="border-0 font-14 font-weight-medium text-muted">Customer Name </th>
	                                    <th class="border-0 font-14 font-weight-medium text-muted">Order No. </th>
	                                    <th class="border-0 font-14 font-weight-medium text-muted">Delivery Time</th>
	                                    <th class="border-0 font-14 font-weight-medium text-muted">
	                                        Service Charge
	                                    </th>
	                                    <th class="border-0 font-14 font-weight-medium text-muted">Order Amount</th>
	...
	<script>
		$(document).ready(function()
		{
			ordersTable = $('#orders').DataTable({ 
			    ...
			    ajax: {
			        ...
			        data: function (d) {
			            ...
			            d.store_id = $("#datatable_store_id option:selected").val();
			            d.status = $("#datatable_order_status option:selected").val();
			        },
			    }, 
			    columns: [
			    // { data: 'id', name: 'id', class:'border-top-0 text-muted px-2 py-4 font-14' },
			    { data: 'customer_name', customer_name: 'customer_name', class:'border-top-0 text-muted px-2 py-4 font-14'  }, 
			    { data: 'order_number', order_number: 'order_number', class:'border-top-0 text-muted px-2 py-4 font-14'  }, 
			    { data: 'delivery_time', delivery_time: 'delivery_time', class:'border-top-0 text-muted px-2 py-4 font-14'  }, 
			    { data: 'service_charges', service_charges: 'service_charges', class:'border-top-0 text-muted px-2 py-4 font-14'  },
			    { data: 'pay_amount', pay_amount: 'pay_amount', class:'border-top-0 text-muted px-2 py-4 font-14'  },
			    { data: 'status', status: 'status', class:'border-top-0 text-muted px-2 py-4 font-14'  },
			...
			$(document).on("click", "#download_all", function(){
			    ...
			    var contest_id = $("#datatable_store_id").val();
