	...
	<div class="page-breadcrumb">
	    <div class="row">
	        ...
	        <div class="col-12 align-self-center customer-leads v3">
	        	<form action="{{ route('customers') }}" id="login" class="filter-form">
	            ...
	            <div class="customize-input float-left Ml20">
	                <select class="custom-select custom-select-set form-control bg-white border-0 custom-shadow custom-radius" name="tag">
								...
								<tbody>
                                	@if(sizeof($customers) > 0)
                                		@foreach ($customers as $key => $customer)
		                                   	<tr class="<?php if($customer->invoices->sum('invoice_amount') > $data['max_value_limit']) echo 'text-danger'; ?>">
		                                        ...
		                                        <td class="border-top-0 px-2 py-4">
		                                            <div class="d-flex no-block align-items-center">
		                                                ...
		                                                <div class="user-detail">
		                                                    ...
		                                                    <h5 class="text-dark mb-0 font-16 font-weight-medium">{{ $customer->name }} {{ $customer->last_name }} - <span class="{{ ($customer->is_reviewed) ? 'reviewed' : 'new' }}">{{ ($customer->is_reviewed) ? 'Reviewed' : 'New' }}</span></h5>
		                                                    </a>
		                                                </div>
		                                            </div>
		                                        </td>
		                                        ...
		                                        <td class="border-top-0 text-muted px-2 py-4 font-14 text-center">{{ $customer->ticket_count }}</td>
		                                        <td class="border-top-0 text-muted px-2 py-4 font-14">{{ $customer->indentification_card }}</td>
		                                       	<!--  <td class="border-top-0 text-muted px-2 py-4 font-14">{{ $customer->approved_by ? $customer->user->name : '' }}</td> -->
		                                        <td class="border-top-0 text-muted px-2 py-4 font-14">
		                                        	<a href="{{ route('invoices', ['customer_id' => $customer->id])  }}" class="btn btn-primary">Receipts</a>
													<a href="{{ route('customer-tickets', ['customer_id' => $customer->id])  }}" class="btn btn-primary">Tickets</a>
		                                        	@if(Auth::user()->user_role == 2)
		                                        		<a href="javascript:void(0);" data-url="{{ route('delete-customer', ['customer_id' => $customer->id])  }}" class="change-psd-btn px-2 delete_customer" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete customer"><i class="fas fa-trash"></i></a>
		                                        	@endif
		                                        </td>
		                                    </tr>
		                                @endforeach
